...
public static class DebugExtension
{
	...
	public static void DebugCapsule(Vector3 start, Vector3 end, Color color, float radius = 1, float duration = 0, bool depthTest = true)
	{
		...
		for(int i = 1; i < 26; i++)
		{
			...
			var c = (i - 1) / 25.0f;
			//Start endcap
			Debug.DrawLine(Vector3.Slerp(right, -up, d)+start, Vector3.Slerp(right, -up, c)+start, color, duration, depthTest);
			Debug.DrawLine(Vector3.Slerp(-right, -up, d)+start, Vector3.Slerp(-right, -up, c)+start, color, duration, depthTest);
			Debug.DrawLine(Vector3.Slerp(forward, -up, d)+start, Vector3.Slerp(forward, -up, c)+start, color, duration, depthTest);
			Debug.DrawLine(Vector3.Slerp(-forward, -up, d)+start, Vector3.Slerp(-forward, -up, c)+start, color, duration, depthTest);
			
			//End endcap
			Debug.DrawLine(Vector3.Slerp(right, up, d)+end, Vector3.Slerp(right, up, c)+end, color, duration, depthTest);
			Debug.DrawLine(Vector3.Slerp(-right, up, d)+end, Vector3.Slerp(-right, up, c)+end, color, duration, depthTest);
			Debug.DrawLine(Vector3.Slerp(forward, up, d)+end, Vector3.Slerp(forward, up, c)+end, color, duration, depthTest);
			Debug.DrawLine(Vector3.Slerp(-forward, up, d)+end, Vector3.Slerp(-forward, up, c)+end, color, duration, depthTest);
		}
	}
